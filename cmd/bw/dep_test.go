...
func TestCmdDepAddBasic(t *testing.T) {
	...
	_, err := cmdDepAdd(env.Store, []string{a.ID, "blocks", b.ID}, PlainWriter(&buf), nil)
	if err != nil {
...
func TestCmdDepAddNotFound(t *testing.T) {
	...
	var buf bytes.Buffer
	_, err := cmdDepAdd(env.Store, []string{"nonexistent", "blocks", "also-missing"}, PlainWriter(&buf), nil)
	if err == nil {
...
func TestCmdDepRemoveBasic(t *testing.T) {
	...
	var buf bytes.Buffer
	_, err := cmdDepRemove(env.Store, []string{a.ID, "blocks", b.ID}, PlainWriter(&buf), nil)
	if err != nil {
...
func TestCmdDepRemoveNotFound(t *testing.T) {
	...
	var buf bytes.Buffer
	_, err := cmdDepRemove(env.Store, []string{"nonexistent", "blocks", "also-missing"}, PlainWriter(&buf), nil)
	if err == nil {
		t.Error("expected error for nonexistent issues")
	}
}
